package com.cyfxr.framework.util;
...
import com.cyfxr.common.utils.IpUtils;
...
public class LogUtils {
    ...
    public static void logAccess(HttpServletRequest request) throws Exception {
        String username = getUsername();//用户名
        String jsessionId = request.getRequestedSessionId();//session Id
        String ip = IpUtils.getIpAddr(request);//ip地址
        String accept = request.getHeader("accept");//头信息
        String userAgent = request.getHeader("User-Agent");//用户代理 识别不同浏览器 电脑和手机端
        String url = request.getRequestURI();
        String params = getParams(request);//参数
    ...
    /**
     * 记录页面错误 错误日志记录 [page/exception][username][statusCode][errorMessage][servletName][uri][exceptionName][ip][exception]
     *
     * @param request
     */
