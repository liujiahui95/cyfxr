...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            exportCyfxExcel: function (excelURL) {
                $.modal.confirm("确定导出" + table.options.modalName + "吗？", function () {
                    var KSRQ = $('#startTime').val();
                    var JSRQ = $('#endTime').val();
                    var yjh = $('#yjh').val();
                    var yj = $('#yj').val();
                    var cz = $('#cz').val();
                    var dw = $('#dw').val();
                    var jm = $('#jm').val();
                    var jldw = $("input:radio:checked").val();
                    var tj = "";//添加所有条件
                    if (KSRQ != null) {
                        tj += "?KSRQ=" + KSRQ
                    }
                    ...
                    alert(tj);
                    $.modal.loading("正在导出数据，请稍后...");
                    window.location.href = ctx + "cyfxExcel/" + excelURL + tj;
                    $.modal.closeLoading();
                });
            },
